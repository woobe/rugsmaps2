...
shinyServer(function(input, output, session) {
  ...
  create_map <- function(map_region = NULL,
    ...
    myCsv <- getURL("https://docs.google.com/spreadsheets/d/1qXr_VBwmrY0uM7m0gTQH_dJLm1faTZAtjs2cdT46kyQ/export?gid=0&format=csv")
    df_rugs <- read.csv(textConnection(myCsv))
    ...
    for (n_rugs in 1:nrow(df_rugs_subset)) {
      ...
      ## Main Group
      tmp_bind <- paste0('Group: <b>', df_rugs_subset[n_rugs,]$MainGroup, '</b><br>')
      
      ## Sub Group if it is availab;e
      if (df_rugs_subset[n_rugs,]$SubGroup != "") {
        tmp_bind <- paste0(tmp_bind, 'Sub Group: <b>', df_rugs_subset[n_rugs,]$SubGroup, '</b><br>')
      }
      
#       ## Members
#       if (!is.na(df_rugs_subset[n_rugs,]$Members)) {
#         tmp_bind <- paste0(tmp_bind, 'Members: <b>', df_rugs_subset[n_rugs,]$Members, '</b><br>')
#       }
      
      ## Location
      if (df_rugs_subset[n_rugs,]$State != "") {  ## for US only
        tmp_bind <- paste0(tmp_bind, 'Location: <b>',
                           df_rugs_subset[n_rugs,]$City, ', ',
                           df_rugs_subset[n_rugs,]$State, ', ',
                           df_rugs_subset[n_rugs,]$Country, '</b><br>')
      } else {
        tmp_bind <- paste0(tmp_bind, 'Location: <b>', 
                           df_rugs_subset[n_rugs,]$City, ', ', 
                           df_rugs_subset[n_rugs,]$Country, '</b><br>')
      ...
      ## Usual Meetup Venue
      if (df_rugs_subset[n_rugs,]$Venue != "") {
        tmp_bind <- paste0(tmp_bind, 'Venue: <b>', df_rugs_subset[n_rugs,]$Venue, '</b><br>')
      }
            
      ## Website
      if (df_rugs_subset[n_rugs,]$Website != "") {
        tmp_bind <- paste0(tmp_bind, 'Website: <b><a href="', 
                           as.character(df_rugs_subset[n_rugs,]$Website), 
                           '" target="_blank" style="text-decoration: none">', 
                           as.character(df_rugs_subset[n_rugs,]$Website),
                           '</a></b><br>')
      }
      
      ## If both Key Sponsor and Website are available
      if ((df_rugs_subset[n_rugs,]$KeySponsor != "") & (df_rugs_subset[n_rugs,]$KeySponsorWebsite != "")) {
        tmp_bind <- paste0(tmp_bind, 'Key Sponsor: <b><a href="', 
                           as.character(df_rugs_subset[n_rugs,]$KeySponsorWebsite), 
                           '" target="_blank style="text-decoration: none"">', 
                           as.character(df_rugs_subset[n_rugs,]$KeySponsor),
                           '</a></b><br>')
      } else if (df_rugs_subset[n_rugs,]$KeySponsor != "") {
        tmp_bind <- paste0(tmp_bind, 'Key Sponsor: <b>', df_rugs_subset[n_rugs,]$KeySponsor, '</b><br>')
      }
            
      ## Add the marker
      map_base$marker(c(df_rugs_subset[n_rugs, ]$lat, df_rugs_subset[n_rugs, ]$lon), bindPopup = tmp_bind)
